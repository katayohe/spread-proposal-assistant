...
使い方:
    ...
    apis = [
        ApiCost(
            ...
            months=6,
            fx_jpy_per_usd=150,
            notes="Bedrock の Claude Sonnet 4.6（ap-northeast-1）を使用（デフォルト）"
        ),
    ]
...
# 第2回SPReAD研究期間の上限（交付決定日〜令和9年2月5日、最大約6ヶ月）
MAX_RESEARCH_MONTHS = 6
...
class ApiCost:
    ...
    api_type_ja: str
    service_name: str      # 裏付け用のサービス名（デフォルト "Amazon Bedrock (Claude Sonnet 4.6)"、ユーザー指定があればそちら）。AWS サービスのみ記載可。サードパーティ API は禁止。
...
def summarize(computes: List[ComputeCost], apis: List[ApiCost]) -> dict:
    ...
    warnings = []
    for c in computes:
        if c.months > MAX_RESEARCH_MONTHS:
            warnings.append(
                f"計算資源 '{c.service_name}': months={c.months} は研究期間上限"
                f"（最大{MAX_RESEARCH_MONTHS}ヶ月）を超えています"
            )
    for a in apis:
        if a.months > MAX_RESEARCH_MONTHS:
            warnings.append(
                f"API '{a.service_name}': months={a.months} は研究期間上限"
                f"（最大{MAX_RESEARCH_MONTHS}ヶ月）を超えています"
            )
    if warnings:
        import sys
...
if __name__ == "__main__":
    ...
    apis = [
        ApiCost(
            ...
            service_name="Amazon Bedrock (Claude Sonnet 4.6)",
            target="科学文献の要約と情報抽出",
            input_tokens_per_month=500_000,
            output_tokens_per_month=100_000,
            input_usd_per_mtok=3.0,
